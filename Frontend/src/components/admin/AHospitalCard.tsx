...
import { getHospital } from "../../api/admin/hospitalService";
...
function AHospitalCard() {
  ...
  useEffect(() => {
    ...
    const fetchHospitalProfile = async () => {
      try {
        setLoading(true);
        // Assuming getHospital is implemented in your hospitalService
      ...
      >
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="relative rounded-lg overflow-hidden">
            {hospitalProfile.bannerImageUrl ? (
              <img
                src={hospitalProfile.bannerImageUrl}
                alt="Banner"
            ...
            <div className="absolute -bottom-12 left-4">
              <div className="w-24 h-24 rounded-full bg-white border-4 border-white flex items-center justify-center">
                {hospitalProfile.profileImageUrl ? (
                  <img
                    src={hospitalProfile.profileImageUrl}
                    alt="Profile"
                ...
                <span
                  className={`px-2 py-1 rounded-full text-xs font-medium ${
                    hospitalProfile.verificationStatus === "verified"
                      ...
                      : hospitalProfile.verificationStatus === "rejected"
                        ? "bg-red-100 text-red-700"
                        : "bg-yellow-100 text-yellow-700"
                  }`}
                >
                  {hospitalProfile.verificationStatus?.toUpperCase() ||
                    "PENDING"}
                </span>
              </div>
            </div>
          ...
          <div className="bg-gray-50 rounded-lg p-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-black font-medium">
                  {hospitalProfile.establishedYear || "-"}
                </p>
              </div>
              ...
              {hospitalProfile.contact?.website && (
                <div>
                  <p className="text-xs text-gray-500 font-medium">Website</p>
                  <a
                    href={
                      hospitalProfile.contact.website.startsWith("http")
                        ? hospitalProfile.contact.website
                        : `https://${hospitalProfile.contact.website}`
                    }
                    target="_blank"
              ...
              {hospitalProfile.contact?.address && (
          ...
          {hospitalProfile.about && (
            <div className="bg-white rounded-lg p-4 border-1 border-gray-200">
              ...
              <p className="text-sm text-gray-700">{hospitalProfile.about}</p>
            </div>
          )}

          {/* Features */}
          {hospitalProfile.features && hospitalProfile.features.length > 0 && (
            <div className="bg-white rounded-lg p-4 border-1 border-gray-200">
              <h3 className="font-semibold text-sm text-gray-700 mb-2 uppercase tracking-wider">
                Features
              </h3>
              <div className="flex flex-wrap gap-2">
                {hospitalProfile.features.map((feature, index) => (
                  <span
                    key={index}
          ...
          <div className="bg-white rounded-lg p-4 border-1 border-gray-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-black font-medium">
                  {hospitalProfile.submissionDate
                    ? new Date(
                        hospitalProfile.submissionDate
                      ).toLocaleDateString()
                    : "-"}
              ...
              {hospitalProfile.verificationRemarks && (
                <div className="md:col-span-2">
        ...
        <div className="flex justify-end gap-3 pt-4 border-t border-gray-200">
          ...
          >
            {hospitalProfile.isBlocked ? "Unblock Hospital" : "Block Hospital"}
          </button>
          <button
            className={`px-4 py-2 rounded-md text-sm font-medium ${
              hospitalProfile.verificationStatus === "verified"
