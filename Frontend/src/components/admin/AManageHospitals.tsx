...
function AManageHospitals() {
  ...
  const setHospitalId = useAdminStore((state) => state.setHospitalId);
  const toggleHospitalCard = useAdminStore((state) => state.toggleHospitalCard);

  function handleRowClick(hospitalId: string) {
  ...
  return (
    <div className="bg-white h-full rounded-lg flex flex-col gap-1.5 border-1 border-gray-300">
      <div className="rounded-t-lg text-black p-3 border-b-1 border-b-gray-300 bg-gray-100 flex lg:justify-between lg:items-center flex-col lg:flex-row gap-2">
        <div className="font-semibold">Hospitals list</div>
          ...
          <tbody>
            {data?.map((hospital) => (
              ...
              >
                ...
                <td>
                  <div>{hospital.isBlocked ? "Blocked" : "Active"}</div>
                </td>
                <td>
                  <div>
...
export default AManageHospitals;
