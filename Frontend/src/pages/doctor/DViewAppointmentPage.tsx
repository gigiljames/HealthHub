...
function DViewAppointmentPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 sm:pt-8 pb-12 space-y-6 w-full">
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-gray-200/40 pb-4 sm:border-b-0 sm:pb-0">
        ...
        <div className="flex flex-wrap items-center gap-2 sm:self-center self-start pl-12 sm:pl-0">
          <span
            className={`px-3 py-1.5 text-xs font-bold rounded-full border ${getStatusBadgeClass(
              appointment.status,
            )}`}
          >
            {appointment.status.replace("_", " ")}
          </span>
          <span className="px-3 py-1.5 text-xs font-bold rounded-full border bg-gray-55 dark:bg-slate-800 text-gray-700 dark:text-slate-300 border-gray-200 dark:border-slate-700 capitalize">
              ...
              <>
                {appointment.rescheduleRequest?.status !== "PENDING" && (
                  <button
                    ...
                    className="px-3.5 py-1.5 bg-emerald-50 text-emerald-600 border border-emerald-200 font-bold rounded-xl hover:bg-emerald-100 dark:bg-emerald-950/20 dark:text-emerald-400 dark:border-emerald-900/50 transition-all text-xs cursor-pointer active:scale-95"
                  >
                    Reschedule
                  </button>
                )}
                <button
                  ...
                  className="px-3.5 py-1.5 bg-red-50 text-red-600 border border-red-200 font-bold rounded-xl hover:bg-red-100 dark:bg-red-955/20 dark:text-red-400 dark:border-red-900/50 transition-all text-xs cursor-pointer active:scale-95"
                >
                  Cancel
                </button>
              </>
          ...
          {(appointment.status === "CONFIRMED" ||
            appointment.status === "IN_PROGRESS") && (
              <button
                ...
                className="px-3.5 py-1.5 bg-darkGreen text-white font-bold rounded-xl hover:bg-darkGreen/90 transition-all text-xs cursor-pointer active:scale-95"
              >
                {appointment.status === "IN_PROGRESS"
                  ? "Rejoin Consultation"
                  : "Join Consultation"}
            ...
            <>
              {reportId && (
                <button
                  ...
                  className="px-3.5 py-1.5 bg-darkGreen hover:bg-darkGreen/90 text-white font-bold rounded-xl transition-all text-xs cursor-pointer active:scale-95"
                >
                  View Report
                </button>
              )}
              {prescriptionId && (
                <button
                  ...
                  className="px-3.5 py-1.5 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20 font-bold rounded-xl transition-all text-xs cursor-pointer active:scale-95"
                >
                  View Prescription
                </button>
              )}
      ...
      {appointment.rescheduleRequest && appointment.rescheduleRequest.status === "PENDING" && (
        <div className="p-4 bg-amber-50 dark:bg-amber-955/20 border border-amber-200 dark:border-amber-900/50 rounded-2xl flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-amber-700 dark:text-amber-400 mt-1 leading-relaxed">
              ...
              <span className="font-semibold text-amber-900 dark:text-amber-200 font-mono">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 flex flex-col gap-6">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-gray-200 dark:border-slate-800 overflow-hidden">
            ...
            <div className="p-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-y-4 sm:gap-y-6 gap-x-4">
                <div>
                  <p className="text-xs sm:text-sm text-gray-500 dark:text-slate-400 mb-0.5">Full Name</p>
                  <p className="font-bold text-gray-800 dark:text-white text-base sm:text-lg">
                    {appointment.patientName || "Unknown"}
                  </p>
                </div>
                <div>
                  <p className="text-xs sm:text-sm text-gray-500 dark:text-slate-400 mb-0.5">Gender</p>
                  <p className="font-medium text-gray-800 dark:text-slate-200 capitalize text-sm sm:text-base">
                    {appointment.gender || "Not specified"}
                  </p>
                </div>
                <div>
                  <p className="text-xs sm:text-sm text-gray-500 dark:text-slate-400 mb-0.5">Age</p>
                  <p className="font-medium text-gray-800 dark:text-slate-200 text-sm sm:text-base">
                    {appointment.dob
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-gray-200 dark:border-slate-800 overflow-hidden">
            ...
            <div className="p-6 flex flex-col gap-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                <div className="flex items-start gap-3 sm:gap-4 p-4 rounded-xl bg-green-50/50 dark:bg-green-900/10 border border-green-100 dark:border-green-900/20">
                  <div className="p-2.5 bg-white dark:bg-slate-800 rounded-lg shadow-sm text-darkGreen shrink-0">
                    {getIcon("clock", "20px", "currentColor")}
                  </div>
                  <div>
                    <p className="text-xs sm:text-sm text-gray-500 dark:text-slate-400 font-medium">
                      Schedule
                    </p>
                    <p className="font-bold text-gray-800 dark:text-white mt-1 text-sm sm:text-base">
                      {dayjs(appointment.start).format(
                        "dddd, MMMM D, YYYY",
                      )}
                    </p>
                    <p className="text-darkGreen font-semibold text-xs sm:text-sm mt-0.5">
                ...
                <div className="flex items-start gap-3 sm:gap-4 p-4 rounded-xl bg-blue-50/50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-900/20">
                  <div className="p-2.5 bg-white dark:bg-slate-800 rounded-lg shadow-sm text-blue-400 shrink-0">
                  ...
                  <div>
                    ...
                    {appointment.location?.address && (
                      <p className="text-gray-600 dark:text-slate-400 text-xs sm:text-sm mt-1 line-clamp-2">
                        {appointment.location.address}
                      </p>
                    )}
                  </div>
              ...
              {appointment.status === "CANCELLED_BY_DOCTOR" && appointment.cancellationReason && (
                <div className="pt-4 border-t border-gray-100 dark:border-slate-800">
                  ...
                  <div className="p-4 bg-red-500/5 border border-red-200 dark:border-red-900/30 rounded-xl text-gray-700 dark:text-slate-300 whitespace-pre-wrap leading-relaxed font-medium italic">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-red-200 dark:border-red-950/40 overflow-hidden">
              <div className="bg-gray-50 dark:bg-slate-800/50 border-b border-gray-200 dark:border-slate-800 px-6 py-4 flex items-center justify-between">
                ...
                <span className={`px-2.5 py-1 text-xs font-semibold rounded-full border uppercase tracking-wider ${dispute.status === "OPEN"
                  ? "bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400 border-blue-200 dark:border-blue-900/40"
                  : dispute.status === "UNDER_REVIEW"
                    ? "bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400 border-yellow-200 dark:border-yellow-900/40"
              ...
              <div className="p-6 space-y-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-700 dark:text-slate-300 bg-gray-50 dark:bg-slate-800/40 p-4 rounded-xl border border-gray-200 dark:border-slate-800 leading-relaxed">
                    {dispute.description}
                  </p>
                </div>
                      ...
                      {dispute.evidence.map((ev: any, idx: number) => {
                        ...
                        return (
                          <a
                            ...
                            className="flex items-center gap-3 p-3 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-800 hover:border-red-300 dark:hover:border-red-900/50 rounded-xl transition-all group hover:shadow-sm"
                          >
                            {isImg ? (
                    ...
                    {previousAppointments.map((apt) => (
                      ...
                      >
                        <td className="py-3 px-4">
                          <p className="text-sm font-medium text-gray-800 dark:text-slate-200 font-sans">
                        ...
                        <td className="py-3 px-4">
                          <span className="capitalize text-xs sm:text-sm font-semibold text-gray-700 dark:text-slate-300">
                            {apt.mode}
                          </span>
                        </td>
                        <td className="py-3 px-4">
                          <span
                            className={`px-2.5 py-1 text-xs font-bold rounded-full border ${getStatusBadgeClass(apt.status)}`}
                          >
                        ...
                        <td className="py-3 px-4">
                          <p
                            className="text-xs sm:text-sm text-gray-600 dark:text-slate-400 max-w-[180px] truncate"
                            title={apt.reason}
                          >
                            {apt.reason || "—"}
                          </p>
                        ...
                        <td className="py-3 px-4">
                          <button
                            ...
                            className="px-3.5 py-1.5 text-xs font-bold bg-darkGreen text-white rounded-xl transition-all shadow-sm active:scale-95 cursor-pointer"
                          >
                            View
                          </button>
                        </td>
                ...
                <div className="flex flex-col gap-5">
                  ...
                  {appointment.refund && (
                    <div className="flex justify-between items-center pb-4 border-b border-gray-100 dark:border-slate-800">
                      ...
                      <span className="px-2.5 py-1 text-xs font-semibold rounded-full border bg-purple-100 text-purple-800 border-purple-200 dark:bg-purple-950/20 dark:text-purple-400 dark:border-purple-900/30">
                        Refunded
                      </span>
                    </div>
                  )}
