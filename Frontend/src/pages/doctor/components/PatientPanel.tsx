...
interface MedicalRecordOverlayProps {
  record: { type: "report" | "prescription"; reportId?: string; appointmentId?: string; prescriptionId?: string };
  onClose: () => void;
}

const MedicalRecordOverlay: React.FC<MedicalRecordOverlayProps> = ({ record, onClose }) => {
  ...
  return (
    <div className="animate-slide-in-right h-full flex flex-col min-h-0 bg-white dark:bg-slate-900">
      ...
      <div className="flex-1 overflow-y-auto pt-4 space-y-5 pr-1">
        {/* Metadata summary (Doctor / Patient / Date) */}
        <div className="p-4 bg-slate-50 dark:bg-slate-800/30 rounded-2xl border border-slate-200/40 dark:border-slate-800/40 space-y-3 shadow-sm">
          <div className="flex justify-between items-start gap-4">
            <div>
              <p className="text-[10px] text-slate-405 uppercase font-black tracking-widest">Consultant</p>
              <p className="text-sm font-extrabold text-slate-850 dark:text-slate-200 mt-0.5">
                Dr. {data.doctorName || "Sarah Connor"}
              </p>
              <span className="inline-block text-[10px] font-bold text-emerald-600 dark:text-emerald-455 uppercase mt-0.5 tracking-wider bg-emerald-500/5 px-2 py-0.5 rounded border border-emerald-500/10">
                {data.doctorSpecialization || "General Medicine"}
              </span>
            </div>
            <div className="text-right">
              <p className="text-[10px] text-slate-405 uppercase font-black tracking-widest">Issue Date</p>
          ...
          <div className="pt-3 border-t border-slate-200/50 dark:border-slate-700/50">
            <p className="text-[10px] text-slate-405 uppercase font-black tracking-widest">Patient Name</p>
            <p className="text-sm font-bold text-slate-800 dark:text-slate-100 mt-0.5">
              {data.patientName || "Sarah Connor"}
            </p>
          </div>
        </div>
...
export const PatientPanel: React.FC<PatientPanelProps> = ({
  ...
  const [activeRecordToView, setActiveRecordToView] = useState<null | { type: "report" | "prescription"; reportId?: string; appointmentId?: string; prescriptionId?: string }>(null);
                    ...
                    <div className="space-y-2 shrink-0">
                      ...
                      <div className="flex gap-2 items-center">
                        <button
                          ...
                          className="text-xs font-bold text-slate-600 hover:text-slate-800 dark:hover:text-slate-350 flex items-center justify-center bg-slate-100/50 dark:bg-slate-800/40 px-3.5 py-2 rounded-lg border border-slate-200/50 dark:border-slate-700/50 transition-all duration-100 animate-fade-in"
                        >
