...
const DChatsPage: React.FC = () => {
  ...
  const isClosedForDoctor = chatStatus?.isClosed && chatStatus.closeReason === "7_DAYS_PASSED";
                    ...
                    {chat.latestMessage ? (
                      <p className="text-xs text-slate-500 dark:text-slate-400 truncate pr-2">
                        <span className="font-semibold">
                          {chat.latestMessage.senderRole === "doctor" ? "You: " : ""}
                        </span>
        ...
        {activeChat ? (
          <>
            ...
            <div className="h-16 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-4 md:px-6 flex items-center justify-between shrink-0">
              ...
              {/* Status info bar */}
              {chatStatus && (
                <div className="hidden lg:flex items-center gap-1.5 bg-slate-101 dark:bg-slate-800 px-3 py-1.5 rounded-full text-xs font-semibold">
                  ...
                  <span className="text-slate-600 dark:text-slate-300">
                    {chatStatus.isClosed
                      ? chatStatus.closeReason === "7_DAYS_PASSED"
                        ? "Chat Closed"
                        : "Patient Message Limit Reached"
                      : `Patient Limit: ${chatStatus.patientMessagesLeft}/30 messages left`}
                  </span>
                </div>
              )}
            </div>
            ...
            {/* Quoted message bar */}
            {replyingToMessage && (
              <div className="p-2 border-t border-slate-200 dark:border-slate-800 bg-slate-101 dark:bg-slate-900 flex justify-between items-center shrink-0">
                <div className="text-[10px] text-slate-500 flex flex-col truncate pr-2">
                  <span className="font-bold text-[9px] text-emerald-600 dark:text-emerald-450">
                    Quoting {replyingToMessage.senderRole === "patient" ? activeChat.recipientName : "You"}:
                  </span>
            ...
            {isClosedForDoctor ? (
              <div className="p-4 border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex items-start gap-3 shrink-0">
                <div className="p-2 bg-red-50 dark:bg-red-955/20 text-red-500 dark:text-red-400 rounded-xl shrink-0 mt-0.5">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-550 dark:text-slate-400 mt-0.5 leading-relaxed">
                    It has been more than 7 days since this consultation ended. Post-consultation chats are limited to a 7-day follow-up window. Doctors can no longer send messages in this thread.
                  </p>
                </div>
              </div>
            ) : (
              <div className="flex flex-col shrink-0">
                {chatStatus?.closeReason === "MESSAGE_LIMIT_REACHED" && (
                  <div className="px-4 py-2 bg-yellow-50 dark:bg-yellow-950/20 text-[11px] text-yellow-800 dark:text-yellow-405 border-t border-yellow-100 dark:border-yellow-900/30 flex items-center gap-1.5">
                    <span className="w-1.5 h-1.5 rounded-full bg-yellow-500"></span>
                    <span>Patient has reached their post-consultation 30-message limit. You can still message them to finalize clinical recommendations.</span>
                  </div>
                )}
                <form
                ...
                >
                  ...
                  <input
                    ...
                    placeholder="Provide clinical instructions or answer follow-ups..."
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-slate-400 p-6 text-center">
            ...
            <p className="text-xs text-slate-550 dark:text-slate-400 mt-1 max-w-[280px] leading-relaxed">
              Choose a patient chat thread from the sidebar list to review message history and follow up on their recovery.
            </p>
          </div>
        )}
      </div>
