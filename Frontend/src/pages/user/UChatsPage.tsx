...
const UChatsPage: React.FC = () => {
    ...
    const loadChatHistory = async () => {
      ...
      try {
        ...
        if (res.success && res.data) {
          ...
          // Queue incoming unread doctor messages as read
          const unreadIds = loadedMsgs
            .filter((msg: any) => msg.senderRole === "doctor" && !msg.readAt)
            .map((msg: any) => msg.id);
  ...
  return (
    <div className="h-screen w-full flex bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-100 font-sans pt-[70px] overflow-hidden">
      ...
      <div className={`flex-1 flex flex-col min-w-0 bg-slate-100/40 dark:bg-slate-950 ${showMobileChatWindow ? "flex" : "hidden md:flex"
        }`}>
        {activeChat ? (
          <>
            {/* Active Header */}
